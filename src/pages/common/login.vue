...
//   loginModel.captchaKey = new Date().getTime()
//   try {
//     const { result } = await getRandomCodeApi(loginModel.captchaKey.toString())
...
async function submit() {
  if (!formRef.value)
    return Promise.reject(new Error('formRef is undefined'))
  submitLoading.value = true
  try {
    await formRef.value?.validate()
    const params: LoginParams = {
      tel: loginModel.username,
      password: loginModel.password,
    } as unknown as LoginParams
    const { token: newToken } = await loginApi(params) as any
    ...
    const redirect = getQueryParam('redirect', '/dashboard/analytics')
    ...
    <div class="login-content flex-center">
      <div class="ant-pro-form-login-main rounded">
        ...
        <div class="flex-between h-15 px-4 mb-[2px]">
          ...
          >
            <carbon-moon v-if="layoutSetting.theme === 'light'" />
            <carbon-sun v-else />
          </span>
          <SelectLang />
        </div> -->
            ...
            <a-form ref="formRef" :model="loginModel">
              ...
              <template v-if="loginModel.type === 'account'">
                <a-form-item name="username" :rules="[{ required: true, message: t('pages.login.username.required') }]">
                  <a-input
                    v-model:value="loginModel.username" allow-clear autocomplete="off"
